...
- **-excluded** `<filename>`: Read types to be excluded from analysis from the specified file. The file should contain
  fully qualified type names, one per line. See the [“Excluding Types”](#excluding-types) section for more details.
...
#### Use a sentinel error variable (most idiomatic)

This is often the clearest and most common Go practice.
...
This program is correct, since the errors are compared by value (and type), and two zero-sized variables of the same
type always compare equal.

#### Make the Error Non-Zero-Sized

If you need to maintain the custom error type structure for specific reasons (e.g., backward compatibility), you can
make it non-zero-sized and optionally provide an `Is` method to restore the previous behavior of `errors.Is` when
comparing against this error type:

```go
type DivisionByZeroError struct{ _ int }

func (*DivisionByZeroError) Is(err error) bool {
...
While this approach is more verbose than using `errors.New` or the original pointer-based zero-sized error
...
This is especially useful when running with the `-fix` flag.
...
If you encounter such a scenario, the recommended approach to manage these legitimate cases is:
