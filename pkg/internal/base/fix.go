...
// ReplaceWithZeroValue generates a suggested fix to replace an pointer expression with its zero-value representation.
func (v *Base) ReplaceWithZeroValue(n ast.Node, t types.Type) []analysis.SuggestedFix {
	var buf bytes.Buffer
	if !v.writeType(&buf, t) {
		return nil
	}
...
func (v *Base) RemoveStar(x ast.Expr) []analysis.SuggestedFix {
	if n, ok := ast.Unparen(x).(*ast.StarExpr); ok {
		v.seen.Insert(n.Pos())

		return v.RemoveOp(n, n.X)
	}
...
func (v *Base) StarSeen(n *ast.StarExpr) bool {
	return v.seen.Has(n.Pos())
}

// RemoveOp suggests a fix that removes a unary operator ('*' or '&') from an expression.
func (v *Base) RemoveOp(n ast.Node, x ast.Expr) []analysis.SuggestedFix {
	var buf bytes.Buffer
...
func (v *Base) MakePure(n ast.Node, x ast.Expr) []analysis.SuggestedFix {
	var buf bytes.Buffer
	if err := format.Node(&buf, v.pass.Fset, x); err != nil {
		return nil
	}
