...
func TestChecker_ReplaceWithZeroValue(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fixes := c.ReplaceWithZeroValue(nodeToReplace, typeOfZeroValue)
			assertFix(t, fixes, tt.expectNilFix, "replace by zero value", tt.expectedNewText)
		})
	}
}

func TestChecker_RemoveStar(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fixes := c.RemoveStar(exprX)
			assertFix(t, fixes, tt.expectNilFix, tt.expectedMessage, tt.expectedNewText)
		})
	}
}

func TestChecker_IgnoreStar(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name         string
		src          string
		findExpr     func(f *ast.File) *ast.StarExpr
		shouldBeSeen bool // indicates if the position was added to the seen set
	}{
		{
			name: "star expression seen",
			src:  "package testpkg\nvar p *int = nil\nvar _ = *p",
			findExpr: func(f *ast.File) *ast.StarExpr {
				valSpec := f.Decls[1].(*ast.GenDecl).Specs[0].(*ast.ValueSpec)

				return valSpec.Values[0].(*ast.StarExpr) // *p
			},
			shouldBeSeen: true,
		},
		{
			name: "star expression not seen",
			src:  "package testpkg\nvar p *int = nil\nvar _ = *p",
			findExpr: func(f *ast.File) *ast.StarExpr {
				valSpec := f.Decls[1].(*ast.GenDecl).Specs[0].(*ast.ValueSpec)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := c.StarSeen(expr)
			if got != tt.shouldBeSeen {
				t.Errorf("StarSeen = %v, want %v", got, tt.shouldBeSeen)
			}
		})
	}
}

func TestChecker_StarSeen(t *testing.T) {
	t.Parallel()
...
func TestChecker_RemoveOp(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fixes := c.RemoveOp(nodeN, exprX)
			assertFix(t, fixes, false, "remove operator", tt.expectedNewText)
		})
	}
}

func TestChecker_MakePure(t *testing.T) {
	t.Parallel()
