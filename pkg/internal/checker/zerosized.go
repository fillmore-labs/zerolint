...
func (c *Checker) ZeroSizedType(t types.Type) (valueMethod, zeroSized bool) {
	...
	// typeCache stores the cached results of zero-sized and value-method checks.
...
// isIgnored checks if a type should be considered for zero-size analysis.
func (c *Checker) isIgnored(t types.Type) bool {
	...
	// Check if a cgo type or the type definition is explicitly ignored.
	return isCtype(tn) || c.isIgnoredType(tn)
}
