...
func newTestChecker(tb testing.TB,
	...
	c.Prepare(&analysis.Pass{
		Pkg:       pkg,
		TypesInfo: info,
		Fset:      fset, // Important for format.Node
	})

	c.Current = currentFile // Important for v.qualifier
...
func parseSourceImports(tb testing.TB, pkgPath string, src string) (*types.Package, *token.FileSet, *ast.File) {
	tb.Helper()
