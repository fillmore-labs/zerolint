...
func (v *visitorInternal) visitCallBasic(n *ast.CallExpr) bool { //nolint:cyclop
	s, ok := ast.Unparen(n.Fun).(*ast.SelectorExpr)
	if !ok {
		return true
	}

	if sel, ok := v.Pass.TypesInfo.Selections[s]; ok {
		...
		case types.MethodExpr:
			...
			v.report(s, message, nil) // Will be fixed by StarExpr.
	...
	pkg, ok := pkgName(v.Pass, s)
	if !ok {
		return true
	}

	switch path := pkg.Imported().Path(); {
	case path == "encoding/json" && s.Sel.Name == "Unmarshal":
