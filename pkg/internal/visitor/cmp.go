...
func (v *visitorInternal) visitCmp(n ast.Node, x, y ast.Expr) bool {
	...
	var message string
	switch {
	case p.left.zeroSizedPonter && p.right.zeroSizedPonter:
		message = p.comparisonMessage()

	case p.left.zeroSizedPonter:
		message = p.comparisonMessageLeft()

	case p.right.zeroSizedPonter:
...
func (v *visitorInternal) operandInfo(x ast.Expr) (operandInfo, bool) {
	...
	case *types.Pointer:
		...
		return operandInfo{zeroSizedPonter: true, elem: t.Elem()}, true // comparisons with a zero-sized pointer
