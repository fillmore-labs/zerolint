...
func (v *Visitor) visitTypeSpec(n *ast.TypeSpec) bool {
	if strings.HasPrefix(n.Name.Name, "_Ctype_") {
		return false
	}

	t := v.check.TypesInfo().TypeOf(n.Type)
	if elem, valueMethod, zeroSized := v.check.ZeroSizedTypePointer(t); zeroSized {
		message := fmt.Sprintf("type declaration to pointer to zero-size variable of type %q", elem)
		fixes := v.check.RemoveStar(n.Type)
		v.check.Report(n, catTypeDeclaration, valueMethod, message, fixes)
	}
