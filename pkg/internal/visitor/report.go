...
func (v *visitorInternal) removeOp(n ast.Node, x ast.Expr) []analysis.SuggestedFix {
...
func (v *visitorInternal) makePure(n ast.Node, x ast.Expr) []analysis.SuggestedFix {
	var buf bytes.Buffer
	if err := format.Node(&buf, v.Pass.Fset, x); err != nil {
		return nil
	}
...
func (v *visitorInternal) report(rng analysis.Range, message string, fixes []analysis.SuggestedFix) {
