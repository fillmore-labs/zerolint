...
func (v *Visitor) checkFieldList(n *ast.FieldList, skipNamed bool, formatter msgFormatter) {
	for _, field := range n.List {
		...
		if elem, valueMethod, zeroSized := v.check.ZeroSizedTypePointer(t); zeroSized {
			cM := formatMessage(formatter, elem, valueMethod, field.Names)
			fixes := v.check.RemoveStar(field.Type)
			v.check.Report(field, cM, fixes)
		}
	}
}
