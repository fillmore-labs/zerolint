...
func (v *Visitor) checkFieldList(n *ast.FieldList, skipNamed bool, msgFormatter checker.MsgFormatter[category]) {
	for _, field := range n.List {
		if skipNamed && len(field.Names) > 0 {
			continue // check only embedded types
		}

		t := v.check.TypesInfo().TypeOf(field.Type)
		if elem, valueMethod, zeroSized := v.check.ZeroSizedTypePointer(t); zeroSized {
			cat, message := checker.FormatMessage(field.Names, msgFormatter, elem)
			fixes := v.check.RemoveStar(field.Type)
			v.check.Report(field, cat, valueMethod, message, fixes)
		}
	}
}
