...
// visitFuncLit examines functions for explicit nil return values for zero-sized types.
...
func (v *Visitor) hasZeroSizedPointerReturn(results *ast.FieldList) ([]retType, bool) {
	...
	for _, res := range results.List {
		...
		t := v.base.TypesInfo().TypeOf(res.Type)
		retInfo.elem, retInfo.valueMethod, retInfo.zeroSized = v.base.ZeroSizedTypePointer(t)
...
func (v *Visitor) visitReturnStmt(n *ast.ReturnStmt, returnTypes []retType) {
	...
	for i, result := range n.Results {
		...
		tv, ok := v.base.TypesInfo().Types[result]
		if !ok {
			continue
		}
		...
		case *types.Basic:
			...
			if t.Kind() == types.UntypedNil {
				message := fmt.Sprintf("explicitly returning nil for pointer to zero-sized type %q", returnType.elem)
				fixes := v.base.ReplaceWithZeroValue(result, returnType.elem)
				v.base.Report(result, catNilReturn, returnType.valueMethod, message, fixes)
			}
