...
func (v *Visitor) visitFuncRecv(n *ast.FuncDecl) {
	...
	r := v.base.TypesInfo().TypeOf(p)

	elem, valueMethod, zeroSized := v.base.ZeroSizedTypePointer(r)
	if !zeroSized {
		return
	}
	...
	fixes := v.base.RemoveStar(p)
	v.base.Report(p, cat, valueMethod, message, fixes)
}
...
func (v *Visitor) isError(name *ast.Ident) bool {
	...
	def := v.base.TypesInfo().Defs[name]
	if def == nil {
		return true
	}
