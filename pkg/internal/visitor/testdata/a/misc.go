...
func (*B) Combine(_ *B) *B { // want "\\(zl:rcv\\)" "\\(zl:arg\\)" "\\(zl:res\\)"
...
var _ fmt.Stringer = (*greeter)(nil) // want "\\(zl:cst\\)"

var _ fmt.Stringer = new(greeter) // want "\\(zl:new\\)"

var _ = (*greeter).String(nil) // want "\\(zl:mex\\)" "\\(zl:par\\)"
