...
var _ = a[0]()`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, info := checkSource(t, fset, []*ast.File{file})
			callExpr := lastDeclCallExpr(file)

			res, ok := FuncOf(info, callExpr)
			...
			if funcName := res.Func.FullName(); funcName != tt.wantFuncName {
				t.Errorf("FuncOf() fun.FullName() = %q, want %q", funcName, tt.wantFuncName)
			}

			if res.MethodExpr != tt.wantMethodExpr {
				t.Errorf("FuncOf() methodExpr = %v, want %v", res.MethodExpr, tt.wantMethodExpr)
			}
		})
	}
}
