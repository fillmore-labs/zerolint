...
import (
	...
	"fillmore-labs.com/zerolint/internal/analyzer/msg"
	"fillmore-labs.com/zerolint/internal/typeutil"
)
...
func (v *Visitor) visitCall(n *ast.CallExpr) bool {
	...
	case funType.IsValue():
		...
		res, ok := typeutil.FuncOf(v.Diag.TypesInfo(), n)
		if !ok { // *ast.FuncLit or *types.Var
			return true
		}

		if isCfunc(res.Func.Name()) {
			return false
		}

		// visitCallFun checks for encoding/json#Decoder.Decode, json.Unmarshal, errors.Is and errors.As.
		if !v.visitCallFunc(n, res.Func, res.MethodExpr) {
			return false
		}

		if res.MethodExpr && v.Level.AtLeast(level.Extended) {
			// Method used as a function value (e.g., (*T).Method).
			v.visitCallMethodExpr(res.SelectorExpr, res.Selection)
		}
