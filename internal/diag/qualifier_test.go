...
import (
	...
	. "fillmore-labs.com/zerolint/internal/diag"
)
...
func TestQualifier_Qualifier(t *testing.T) { //nolint:funlen
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pkg, _, file := parseSourceImports(t, tt.currentPkgPath, tt.currentFileSrc)
			q := Qualifier{
				Pkg:     pkg,
				Imports: tt.importsProvider(file),
			}

			targetPkgToQualify := tt.targetPkgProvider(pkg)
