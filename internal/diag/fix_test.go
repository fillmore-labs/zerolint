...
func TestDiag_ReplaceWithZeroValue(t *testing.T) { //nolint:funlen
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			info, pkg, fset, file := parseSource(t, "test.go", tt.src)
			d := newTestDiag(t, info, pkg, fset, file)
			nodeToReplace, typeOfZeroValue := tt.findNodeAndType(file, pkg, info)
...
func TestDiag_RemoveOp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			info, pkg, fset, file := parseSource(t, "test.go", tt.src)
			d := newTestDiag(t, info, pkg, fset, file)
			nodeN, exprX := tt.findNodes(file, info)
